...
(function () {
  ...
  function _arrayWithoutHoles(arr) {
    if (Array.isArray(arr)) {
      for (var i = 0, arr2 = Array.from({ length: arr.length }); i < arr.length; i++) arr2[i] = arr[i]
  ...
  const RESERVED_CLASSES = ['xs', 'sm', 'lg', 'fw', 'ul', 'li', 'border', 'pull-left', 'pull-right', 'spin', 'pulse', 'rotate-90', 'rotate-180', 'rotate-270', 'flip-horizontal', 'flip-vertical', 'flip-both', 'stack', 'stack-1x', 'stack-2x', 'inverse', 'layers', 'layers-text', 'layers-counter', DUOTONE_CLASSES.GROUP, DUOTONE_CLASSES.SWAP_OPACITY, DUOTONE_CLASSES.PRIMARY, DUOTONE_CLASSES.SECONDARY].concat(oneToTen.map((n) => {
    return ''.concat(n, 'x')
  })).concat(oneToTwenty.map((n) => {
    return 'w-'.concat(n)
  }))
  ...
  function classArray(node) {
    if (node.classList) {
      ...
      return (node.getAttribute('class') || '').split(' ').filter((i) => {
        return i
      })
    }
  }
  ...
  function makeInlineSvgAbstract(params) {
    ...
    const attrClass = [config.replacementClass, iconName ? ''.concat(config.familyPrefix, '-').concat(iconName) : '', widthClass].filter((c) => {
  ...
  function getCanonicalIcon(values) {
    return values.reduce((acc, cls) => {
      ...
      else if (config.autoFetchSvg && Object.keys(PREFIX_TO_STYLE).includes(cls)) {
        acc.prefix = cls
      }
      else if (iconName) {
  ...
  var mutators = {
    replace: function replace(mutation) {
      ...
      const newOuterHTML = abstract.map((a) => {
        return toHtml(a)
      }).join('\n')
    ...
    nest: function nest(mutation) {
      ...
      const newInnerHTML = abstract.map((a) => {
        return toHtml(a)
      }).join('\n')
  ...
  function maskParser(node) {
    ...
    if (!mask) {
      ...
      return getCanonicalIcon(mask.split(' ').map((i) => {
        return i.trim()
      }))
    }
  }
  ...
  function onTree(root) {
    ...
    const prefixesDomQuery = ['.'.concat(LAYERS_TEXT_CLASSNAME, ':not([').concat(DATA_FA_I2SVG, '])')].concat(prefixes.map((p) => {
  ...
  function replaceForPosition(node, position) {
    ...
    return new picked((resolve, reject) => {
      ...
      const alreadyProcessedPseudoElement = children.filter((c) => {
      ...
      if (alreadyProcessedPseudoElement && !fontFamily) {
        ...
        if (iconName && (!alreadyProcessedPseudoElement || alreadyProcessedPseudoElement.getAttribute(DATA_PREFIX) !== prefix || alreadyProcessedPseudoElement.getAttribute(DATA_ICON) !== iconIdentifier)) {
          ...
          findIcon(iconName, prefix).then((main) => {
            ...
            element.outerHTML = abstract.map((a) => {
              return toHtml(a)
            }).join('\n')

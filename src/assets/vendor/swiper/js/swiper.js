...
: window; // eslint-disable-line
  ...
  function outerWidth(includeMargins) {
    if (this.length > 0) {
      if (includeMargins) {
        ...
        return this[0].offsetWidth + Number.parseFloat(styles.getPropertyValue('margin-right')) + Number.parseFloat(styles.getPropertyValue('margin-left'))
      }
  ...
  function outerHeight(includeMargins) {
    if (this.length > 0) {
      if (includeMargins) {
        ...
        return this[0].offsetHeight + Number.parseFloat(styles.getPropertyValue('margin-top')) + Number.parseFloat(styles.getPropertyValue('margin-bottom'))
      }
  ...
  var Utils = {
    ...
    getTranslate: function getTranslate(el, axis) {
      ...
      if (win.WebKitCSSMatrix) {
        ...
        if (curTransform.split(',').length > 6)
          curTransform = curTransform.split(', ').map((a) => { return a.replace(',', '.') }).join(', ')
    ...
    parseUrlQuery: function parseUrlQuery(url) {
      ...
      if (typeof urlToParse === 'string' && urlToParse.length) {
        urlToParse = urlToParse.includes('?') ? urlToParse.replace(/\S*\?/, '') : ''
        params = urlToParse.split('&').filter((paramsPart) => { return paramsPart !== '' })
  ...
  SwiperClass.use = function use(module) {
    ...
    if (Array.isArray(module)) {
      module.forEach((m) => { return Class.installModule(m) })
  ...
  function updateSize() {
    ...
    // Subtract paddings
    width = width - Number.parseInt($el.css('padding-left'), 10) - Number.parseInt($el.css('padding-right'), 10)
    height = height - Number.parseInt($el.css('padding-top'), 10) - Number.parseInt($el.css('padding-bottom'), 10)
      ...
      if (params.slidesPerView === 'auto') {
        ...
        if (params.roundLengths) {
          ...
          if (swiper.isHorizontal()) {
            const width = Number.parseFloat(slideStyles.getPropertyValue('width'))
            const paddingLeft = Number.parseFloat(slideStyles.getPropertyValue('padding-left'))
            const paddingRight = Number.parseFloat(slideStyles.getPropertyValue('padding-right'))
            const marginLeft = Number.parseFloat(slideStyles.getPropertyValue('margin-left'))
            const marginRight = Number.parseFloat(slideStyles.getPropertyValue('margin-right'))
            ...
            const height = Number.parseFloat(slideStyles.getPropertyValue('height'))
            const paddingTop = Number.parseFloat(slideStyles.getPropertyValue('padding-top'))
            const paddingBottom = Number.parseFloat(slideStyles.getPropertyValue('padding-bottom'))
            const marginTop = Number.parseFloat(slideStyles.getPropertyValue('margin-top'))
            const marginBottom = Number.parseFloat(slideStyles.getPropertyValue('margin-bottom'))
  ...
  function updateActiveIndex(newActiveIndex) {
    ...
    // Get real index
    const realIndex = Number.parseInt(swiper.slides.eq(activeIndex).attr('data-swiper-slide-index') || activeIndex, 10)
  ...
  function updateClickedSlide(e) {
    ...
    if (slide && slideFound) {
      ...
      if (swiper.virtual && swiper.params.virtual.enabled)
        swiper.clickedIndex = Number.parseInt($(slide).attr('data-swiper-slide-index'), 10)
      else
  ...
  function slidePrev(speed, runCallbacks, internal) {
    ...
    const normalizedSnapGrid = snapGrid.map((val) => { return normalize(val) })
    const normalizedSlidesGrid = slidesGrid.map((val) => { return normalize(val) })
  ...
  function slideToClickedSlide() {
    ...
    if (params.loop) {
      if (swiper.animating)
        return
      realIndex = Number.parseInt($(swiper.clickedSlide).attr('data-swiper-slide-index'), 10)
      if (params.centeredSlides) {
        if (
  ...
  function loopCreate() {
    ...
    swiper.loopedSlides = Math.ceil(Number.parseFloat(params.loopedSlides || params.slidesPerView, 10))
  ...
  const Device = (function Device() {
    ...
    const ie = ua.includes('MSIE ') || ua.includes('Trident/')
    const edge = ua.includes('Edge/')
    const firefox = ua.includes('Gecko/') && ua.includes('Firefox/')
    const windows = platform === 'Win32'
    const electron = ua.toLowerCase().includes('electron')
  ...
  function setBreakpoint() {
    ...
    let loopedSlides = swiper.loopedSlides; if (loopedSlides === void 0)
      loopedSlides = 0
    const params = swiper.params
    const $el = swiper.$el
      ...
      if (breakpointOnlyParams) {
        ['slidesPerView', 'spaceBetween', 'slidesPerGroup', 'slidesPerColumn'].forEach((param) => {
          ...
          if (param === 'slidesPerView' && (paramValue === 'AUTO' || paramValue === 'auto'))
            ...
            breakpointOnlyParams[param] = Number.parseFloat(paramValue)
          else
            breakpointOnlyParams[param] = Number.parseInt(paramValue, 10)
        })
      }
  ...
  function getBreakpoint(breakpoints) {
    ...
    points.sort((a, b) => { return Number.parseInt(a, 10) - Number.parseInt(b, 10) })
  ...
  const Virtual = {
    update: function update(force) {
      ...
      prependIndexes.sort((a, b) => { return b - a }).forEach((index) => {
    ...
    prependSlide: function prependSlide(slides) {
      ...
      if (swiper.params.virtual.cache) {
        ...
        Object.keys(cache).forEach((cachedIndex) => {
          ...
          if (cachedElIndex)
            $cachedEl.attr('data-swiper-slide-index', Number.parseInt(cachedElIndex, 10) + 1)

          newCache[Number.parseInt(cachedIndex, 10) + numberOfNewSlides] = $cachedEl
        })
  ...
  const Scrollbar = {
    ...
    updateSize: function updateSize() {
      ...
      if (swiper.params.scrollbar.dragSize === 'auto')
        ...
        dragSize = Number.parseInt(swiper.params.scrollbar.dragSize, 10)
  ...
  const Parallax = {
    setTransform: function setTransform(el, progress) {
      ...
      if ((x).includes('%'))
        x = `${Number.parseInt(x, 10) * progress * rtlFactor}%`
      else
        x = `${x * progress * rtlFactor}px`

      if ((y).includes('%'))
        y = `${Number.parseInt(y, 10) * progress}%`
      else
        y = `${y * progress}px`
    ...
    setTransition: function setTransition(duration) {
      ...
      $el.find('[data-swiper-parallax], [data-swiper-parallax-x], [data-swiper-parallax-y], [data-swiper-parallax-opacity], [data-swiper-parallax-scale]')
        .each((index, parallaxEl) => {
          ...
          let parallaxDuration = Number.parseInt($parallaxEl.attr('data-swiper-parallax-duration'), 10) || duration
          if (duration === 0)
            parallaxDuration = 0
  ...
  var History = {
    ...
    getPathValues: function getPathValues() {
      const pathArray = win.location.pathname.slice(1).split('/').filter((part) => { return part !== '' })
  ...
  const Cube = {
    setTranslate: function setTranslate() {
      ...
      for (let i = 0; i < slides.length; i += 1) {
        ...
        let slideIndex = i
        if (isVirtual)
          slideIndex = Number.parseInt($slideEl.attr('data-swiper-slide-index'), 10)
  ...
  const Thumbs = {
    ...
    onThumbClick: function onThumbClick() {
      ...
      if (thumbsSwiper.params.loop)
        slideToIndex = Number.parseInt($(thumbsSwiper.clickedSlide).attr('data-swiper-slide-index'), 10)
      else
    ...
    update: function update(initial) {
      ...
      if (swiper.realIndex !== thumbsSwiper.realIndex) {
        ...
        if (thumbsSwiper.visibleSlidesIndexes && !thumbsSwiper.visibleSlidesIndexes.includes(newThumbsIndex)) {
